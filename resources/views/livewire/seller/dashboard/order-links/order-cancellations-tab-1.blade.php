<div class="bg-white overflow-x-auto rounded-lg p-3">
    ...
    <div wire:loading.remove x-transition>
        @if ($this->getCancellationApproved->count() > 0)
            @foreach ($this->getCancellationApproved as $key => $cancellation)
                <div class="border-b border-gray-100" x-data="{ selected: null }">
                    {{-- @dd($cancellation) --}}
                    <div class="grid grid-cols-12 text-center">
                        <div class="col-span-2 my-4 text-sm !text-gray-800 !font-light">
                            {{ $cancellation->reference_number }}
                        </div>
                    ...
                    <div class="relative overflow-hidden transition-all max-h-0 duration-500 ease-in-out bg-background-light rounded"
                        ...
                        <div class="overflow-hidden border-t-2 border-blue-100">
                                                ...
                                                <div class="grid lg:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <input type="text" id="payment_type"
                                                            value="{{ date('M d, Y (h:i a)', strtotime($cancellation->purchase_date)) }}"
                                                            class="bg-transparent !border-b-2 border-gray-600 text-gray-900 focus:!ring-0 focus:border-0 block w-full !p-1"
                                                            placeholder="" disabled>
                                                    </div>
                                                    <div>
                                                        ...
                                                        @if ($cancellation->purchase_status == 'completed')
                                                            <input type="text" id="reference_code"
                                                                value="{{ date('M d, Y (h:i a)', strtotime($cancellation->completion_date)) }}"
                                                                class="bg-transparent !border-b-2 border-gray-600 text-gray-900 focus:!ring-0 focus:border-0 block w-full !p-1"
                                                                placeholder="" disabled>
                                                        @else
                                        ...
                                        <div class="p-1.5 lg:w-1/3">
                                            <div class="mb-2">
                                                <div>
                                                    ...
                                                    @if ($cancellation->payment->payment_type == 'cod')
                                                        <input type="text" id="purchase_date"
                                                            value="COD (Cash On Delivery)"
                                                            class="bg-transparent !border-b-2 border-gray-600 text-gray-900 focus:!ring-0 focus:border-0 block w-full !p-1"
                                                            placeholder="" disabled>
                                                    @elseif ($cancellation->payment->payment_type == 'gcash')
                                        ...
                                        <div class="p-1.5 lg:w-1/3">
                                            <div class="mb-2">
                                                <div>
                                                    ...
                                                    @if ($cancellation->payment->date_of_payment)
                                                        <input type="text" id="purchase_date"
                                                            value="{{ date('M d, Y (h:i a)', strtotime($cancellation->payment->date_of_payment)) }}"
                                                            class="bg-transparent !border-b-2 border-gray-600 text-gray-900 focus:!ring-0 focus:border-0 block w-full !p-1"
                                                            placeholder="" disabled>
                                                    @else
                                        ...
                                        <div class="p-1.5 lg:w-1/3">
                                            <div class="mb-2">
                                                <div>
                                                    ...
                                                    <input type="text" id="purchase_date"
                                                        value="{{ $cancellation->payment->reference_code }}"
                                                        class="bg-transparent !border-b-2 border-gray-600 text-gray-900 focus:!ring-0 focus:border-0 block w-full !p-1"
                                                        placeholder="" disabled>
                                                </div>
                                            </div>
                            ...
                            {{-- @dd($purchase) --}}
                            @foreach ($cancellation->purchase_items as $item)
                                {{-- @dd($item) --}}
